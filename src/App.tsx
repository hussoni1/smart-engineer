...
function DataLabPage({ user, onNavigate, onLogout }: { user: User | null; onNavigate: (path: string) => void; onLogout: () => void }) {
  const [rows, setRows] = useState<string[][]>([]); const [fileName, setFileName] = useState(""); const [message, setMessage] = useState("Upload a CSV file to begin · ارفع ملف CSV للبدء");
  ...
  const analyze = () => { if (rows.length < 2) return setMessage("Upload a CSV first · ارفع ملف CSV أولًا"); const headers = rows[0]; const numeric = headers.map((_, i) => rows.slice(1).map((row) => Number(row[i])).filter((value) => Number.isFinite(value))); const summary = numeric.map((values, i) => values.length ? `${headers[i]}: mean ${(values.reduce((a, b) => a + b, 0) / values.length).toFixed(2)}` : null).filter(Boolean).join(" · "); setMessage(`Analysis complete · اكتمل التحليل: ${summary || "No numeric columns · لا توجد أعمدة رقمية"}`); };
  ...
  return (
    <header className="topbar">
      ...
      <nav className={menuOpen ? "menu-open" : ""}>
        ...
        <button className={active === "data-lab" ? "active" : ""} onClick={() => go("/data-lab")}>مختبر البيانات</button>
        <button className={active === "glossary" ? "active" : ""} onClick={() => go("/glossary")}>قاموس AI</button>
        <button className={active === "exams" ? "active" : ""} onClick={() => go("/exams")}>الامتحانات</button>
        <button className={active === "goals" ? "active" : ""} onClick={() => go("/goals")}>اختر هدفك</button>
