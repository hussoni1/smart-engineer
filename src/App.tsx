...
function DataLabPage({ user, onNavigate, onLogout }: { user: User | null; onNavigate: (path: string) => void; onLogout: () => void }) {
  const [rows, setRows] = useState<string[][]>([]); const [fileName, setFileName] = useState(""); const [message, setMessage] = useState("Upload a CSV file to begin · ارفع ملف CSV للبدء"); const [modelResult, setModelResult] = useState("");
  ...
  const trainBaseline = () => { const numeric = numericColumns(); if (numeric.length < 1) return setModelResult("Need a numeric target · نحتاج عمودًا رقميًا"); const target = numeric[numeric.length - 1].values; const mean = target.reduce((a, b) => a + b, 0) / target.length; const mae = target.reduce((sum, value) => sum + Math.abs(value - mean), 0) / target.length; setModelResult(`Baseline predicts the mean of ${numeric[numeric.length - 1].header}: ${mean.toFixed(2)} · MAE: ${mae.toFixed(2)}`); };
